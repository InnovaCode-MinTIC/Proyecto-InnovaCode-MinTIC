...
public class InnovaCodeApplication {
	...
	public static void testingEnterprises(){
		...
		for (int i = 0; i<10 ; i++) {
			enterprises.add(new Enterprise(i,"Empresa "+i,""+i+i+i+"."+i+i+i+"."+i+i+i+"-"+i,
					"+57 "+i+i+i+i+i+i+i+i+i+i,"Calle "+i+" #"+i+" - "+i));
		}
		...
		System.out.println("Direccion original: "+ enterprises.get(1).getAddress());
		enterprises.get(1).setAddress("Carrera 1 # 2 - 3");
		System.out.println("Nueva Direccion: " + enterprises.get(1).getAddress());
		System.out.println();
